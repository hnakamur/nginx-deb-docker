...
build-debian12: buildkit-logunlimited
	...
	sudo xz --force nginx-${PKG_VERSION}-${PKG_REL_PREFIX}debian12/nginx_${PKG_VERSION}-${PKG_REL_PREFIX}${PKG_REL_DISTRIB}.build.log
...
deb-ubuntu1804: build-ubuntu1804
	docker run --rm -v ./nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04:/dist nginx-ubuntu1804 bash -c \
	"cp /src/*${PKG_VERSION}* /dist/"
	docker run --rm -it nginx-ubuntu1804 /src/run-nginx-tests.sh 2>&1 | sudo tee ./nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04/nginx-tests-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04.log
	sudo xz --force ./nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04/nginx-tests-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04.log
	sudo tar zcf nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04.tar.gz ./nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04/

build-ubuntu1804: buildkit-logunlimited
	sudo mkdir -p nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04
	...
	docker buildx build --progress plain --builder ${LOGUNLIMITED_BUILDER} --load \
		...
		--build-arg PKG_VERSION=${PKG_VERSION} \
		--build-arg LUAJIT_DEB_VERSION=${LUAJIT_DEB_VERSION} \
		--build-arg LUAJIT_DEB_OS_ID=ubuntu18.04 \
		--build-arg MODSECURITY_DEB_VERSION=${MOSECURITY_DEB_VERSION} \
		--build-arg MODSECURITY_DEB_OS_ID=ubuntu18.04 \
		-t nginx-ubuntu1804 . \
	) 2>&1 | sudo tee nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04/nginx_${PKG_VERSION}-${PKG_REL_PREFIX}${PKG_REL_DISTRIB}.build.log && \
	sudo xz --force nginx-${PKG_VERSION}-${PKG_REL_PREFIX}ubuntu18.04/nginx_${PKG_VERSION}-${PKG_REL_PREFIX}${PKG_REL_DISTRIB}.build.log
